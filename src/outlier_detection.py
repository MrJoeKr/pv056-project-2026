...
from src.convae import ImageDataset, train_convautoencoder
...
def detect_outliers_convautoencoder(
    ...
    The method trains a ConvAE on the provided images and applies adaptive
    thresholding (elbow detection with MAD fallback) on reconstruction errors
    to flag outliers — the same strategy used by detect_outliers_knn.
    ...
    elbow_idx, elbow_score, elbow_strength = _detect_elbow_in_scores(distances, min_samples=5)

    if elbow_strength > 0.15 and elbow_idx >= int(0.90 * len(distances)):
        threshold = elbow_score
        method_used = "elbow"
    else:
        threshold = _compute_mad_threshold(distances, multiplier=4.5, use_log=False)
        elbow_idx = -1
        elbow_strength = 0.0
        method_used = "mad"
